...
class QwenClient:
    ...
    **盘前分析 "8问" 深度协议 (The Pre-Market 8 Questions Protocol)**:
    在做出任何交易决策前，必须对以下 8 个问题进行深度分析。分析必须结合**实时市场数据 (Market Data)**、**远程数据库全量历史数据 (Remote DB History)** 以及 **前一次分析结果 (Previous Analysis)** 进行对比。

    1. **历史与现状对比 (History vs Now)**:
       - *当前价格与历史高点/低点相比处于什么位置？* (参考 Remote DB History)
       - *与上一次分析 (Previous Analysis) 相比，市场结构 (Structure) 是否发生了根本性变化？* (例如：之前是 Bullish，现在是否转为 Bearish？)
    2. **趋势健康度 (Trend Health)**:
       - *当前趋势是刚刚开始、正在加速还是接近衰竭？* (参考 ADX, EMA 斜率)
       - *量价配合是否健康？* (参考 Volume, OBV)
    3. **关键区域验证 (Key Levels)**:
       - *价格是否正在测试关键的 HTF (高时间周期) 支撑/阻力位？* (参考 H1/H4 OB, FVG)
       - *这些区域在历史上是否多次有效？*
    4. **流动性状态 (Liquidity State)**:
       - *最近是否有明显的 Liquidity Sweep (流动性扫荡)？* (参考 Swing High/Low)
       - *上方/下方是否存在未回补的 FVG 或待猎杀的止损盘？*
    5. **波动性与风险 (Volatility & Risk)**:
       - *当前 ATR (波动率) 是否异常？* (过高需降低仓位，过低需警惕假突破)
       - *即将到来的新闻事件 (News) 风险如何？*
    6. **相关性分析 (Correlation)**:
       - *对于 ETHUSD，比特币 (BTC) 走势如何？对于 XAUUSD，美元指数 (DXY) 走势如何？*
    7. **盈亏比评估 (Risk/Reward)**:
       - *如果现在入场，潜在的 TP 空间是否至少是 SL 空间的 1.5 倍？* (参考 MFE/MAE 数据)
    8. **执行信心 (Confidence)**:
       - *基于以上所有分析，对这笔交易的信心评分 (0-10) 是多少？为什么？*
...
1    - **MFE 深度优化 (Finalizing TP)**:
    ...
    **交易员与风控团队必须严格遵守的【单边趋势交易技术规范 (Trend Only)】**:
    ...
    3. 关键支撑阻力位
        ...
        analysis_framework = """
    ### 一、交易系统核心架构 (M1 Scalping + Multi-TF Resonance)
    你必须基于以下统一的交易周期架构进行分析 (查看提供的 `multi_tf_data` 和 `sr_resonance`)：
    
    1. **核心周期 (Core Timeframe)**: **M1 (1分钟)**
       - 这是你的**主要战场**。所有的入场信号 (Entry Signal) 必须在 M1 上确认。
       - 寻找微观结构的 BOS, CHOCH, FVG 回补。
    
    2. **辅助共振周期 (Auxiliary Resonance)**: **M5 (5分钟) & M15 (15分钟)**
       - 用于过滤噪音和确认趋势稳定性。
       - **强制共振规则**: 只有当 M1 的信号与 M5 和 M15 的方向/结构一致时，才允许交易。
       - **S/R 共振**: 必须检查 `sr_resonance` 数据。只有在 M1, M5, M15 三个周期上同时存在“有效支撑/阻力”的区域，才是高胜率区域。

    ### 二、严格过滤器 (Strict Filters)
    **任何不满足以下条件的信号，必须直接丢弃 (Action = HOLD)**：
    
    1. **盈亏比硬性指标 (Risk:Reward Hard Filter)**
       - **规则**: `Potential Profit / Potential Loss >= 1.5`
       - 计算: (TP距离 / SL距离) 必须 >= 1.5。
       - 如果 R:R < 1.5，**禁止开仓**。
    
    2. **S/R 有效性定义 (Valid S/R)**
       - 定义: 过去 20 根 K 线内，至少经过 2 次测试，且发生明显价格反转的区域。
       - 检查: 查看输入的 `sr_resonance` 数据，确认当前价格是否位于这样的共振区域附近。
    
    3. **质量评分 (Quality Score)**
       - 你必须对每个交易机会进行 0-100 分的量化打分。
       - **评分标准**:
         - 趋势一致性 (30分): M1/M5/M15 趋势是否同向？
         - S/R 共振强度 (30分): 是否在三周期共振支撑/阻力位？
         - 盈亏比空间 (20分): R:R 是否 > 2.0 (满分) 或 > 1.5 (及格)？
         - 动量/SMC结构 (20分): 是否有清晰的 BOS/CHOCH 或 FVG？
       - **阈值**: `Score < 80` 的信号自动丢弃 (Action = HOLD)。只有 Score >= 80 才能输出 BUY/SELL。

    ### 三、核心规则：盘前交易计划 8 问 (The 8 Pre-Market Questions)
    **在做出任何入场决定 (Action != HOLD) 前，必须严格自我反思并回答以下 8 个问题。若任意一条不满足，强制 HOLD。**
    ...
    6. **你是顺哪个大级别，逆哪个小级别周期?** (Cycle Alignment)
       - **顺大**: 顺应 M15/M5 级别趋势。
       - **逆小**: 在 M1 级别寻找反向回调(Pullback)结束的转折点入场。
    7. **你的防守位在哪里?** (Defense)
       - 必须在开仓前确定。
       - 参考: 极值点、大级别中枢边缘、EMA 慢线。
    8. **要不要到交易周期里去?** (Execution Decision)
       - **Yes**: 前 7 问答案清晰一致，且 M1 级别出现反转信号，且 **Score >= 80**。
       - **No**: 任意答案模糊或逻辑矛盾 -> **HOLD**。
    ...
    **一致性检查 (Consistency Check)**:
    ...
    - **quality_score**: int (0-100, 必须 >= 80 才能 BUY/SELL)
    - **rr_ratio**: float (Risk Reward Ratio, 必须 >= 1.5 才能 BUY/SELL)
        ...
        ** CRITICAL INSTRUCTION **
        ...
        2. **交易商特性与风险适配 (Broker Specifics)**:
           ...
           - **ETHUSD 特别注意**: 
             ...
             - **实时点差成本 (Spread Cost)**: 必须参考 `spread_info` 数据。Basket TP 的下限必须 > (SpreadCost * 2)，否则无法覆盖成本。如果点差过大 (例如 > $3)，请暂停 Scalping 策略。

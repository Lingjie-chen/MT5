...
1    - **MFE 深度优化 (Finalizing TP)**:
    ...
    **交易员与风控团队必须严格遵守的【单边趋势交易技术规范 (Trend Only)】**:
    ...
    3. 关键支撑阻力位
        ...
        analysis_framework = """
    ### 一、交易系统核心架构 (M15 Trend Following + Multi-TF Resonance)
    你必须基于以下统一的交易周期架构进行分析 (查看提供的 `multi_tf_data` 和 `sr_resonance`)：
    
    1. **核心周期 (Core Timeframe)**: **M15 (15分钟)**
       - 这是你的**主要战场**。所有的入场信号 (Entry Signal) 必须在 M15 上确认。
       - 寻找微观结构的 BOS, CHOCH, FVG 回补。
    
    2. **辅助共振周期 (Auxiliary Resonance)**: **H1 (1小时) & H4 (4小时)**
       - 用于过滤噪音和确认趋势稳定性。
       - **强制共振规则**: 只有当 M15 的信号与 H1 和 H4 的方向/结构一致时，才允许交易。
       - **S/R 共振**: 必须检查 `sr_resonance` 数据。只有在 M15, H1, H4 三个周期上同时存在“有效支撑/阻力”的区域，才是高胜率区域。
    ...
    **在做出任何入场决定 (Action != HOLD) 前，必须严格自我反思并回答以下 8 个问题。若任意一条不满足，强制 HOLD。**
    ...
    8. **要不要到交易周期里去?** (Execution Decision)
       - **Yes**: 前 7 问答案清晰一致，且 M15 级别出现反转信号，且 **Score >= 80**。
       - **No**: 任意答案模糊或逻辑矛盾 -> **HOLD**。

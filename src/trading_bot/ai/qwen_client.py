...
class QwenClient:
    ...
    **重要指令: 采用自适应交易 (Adaptive Trading) 风格，在 M15 顺势前提下，利用 M5 波动进行交易。**
    - **禁止**使用盲目网格策略 (Blind Grid Strategy) 和逆势加仓 (Martingale)。
    ...
    - **自适应切换**: 根据市场状态灵活切换 "Trend Following" 或 "Grid Consolidation"。

    **交易节奏控制 (Trend Cycle Control)**:
    - **积极寻找机会**: 在 M15 趋势明确时，利用 M5 的每一个有效回撤或突破进行交易。
    - **自适应模式**: 
      - **趋势模式**: 以 M15 为主趋势，M5 为入场及波段操作周期。
      - **震荡模式**: 利用布林带/ATR 边界进行高抛低吸。
...
1    - **MFE 深度优化 (Finalizing TP)**:
    ...
    **1. 分析师团队 (The Analyst Team)**
    ...
    - **技术分析师**:
        ...
        - **指令**:
            1. **SMC 结构分析**: 重点识别 M5 (入场) 和 M15 (趋势) 的 CHOCH 和 BOS，确认趋势结构变化。
    ...
    **核心规则**: 
    - **开仓**: 必须严格基于 `grid_strategy` 提供的 ORB 信号。无 ORB 信号 = **HOLD**。
    - **例外**: 仅在需要紧急平仓 ('close') 时可不依赖 ORB。
            """
    ...
    *   **Position Sizing**: **完全由大模型分析判断**。你必须基于 M5 (执行) 和 M15 (趋势) 的市场情绪和技术形态，计算出精确的仓位 (Lots)。
    ...
    3. **市场分析要求**:
       ...
       - 必须明确指出最近的 **BOS** 和 **CHoCH** 位置。
        ...
        "DEFAULT": """
    **交易员与风控团队必须严格遵守的【自适应交易技术规范 (Adaptive Mode)】**:
    ...
    5. **入场验证 (Entry Validation - CRITICAL)**:
       ...
       - **必须验证有效性**:
           1. **K线形态**: 必须等待当前时间周期 (M5) 收盘，出现明显的**反转信号** (Pinbar/Hammer/Engulfing)。
    ...
    6. **盘前 8 问严格执行协议 (Pre-Market 8 Questions Protocol)**:
       ...
       - **Q8 执行 (Execution) [CRITICAL]**:
          - **Yes (立即执行)**: 必须满足以下 **优先级** 条件之一：
             ...
             2. **Grid 触发**: ORB 条件不满足，市场处于震荡区间。只要确认是震荡，即可启动 `GRID_START`。
       ...
       - **Q9 风险预算检查 (Risk Budget Check)**:
          ...
          - **Pass**: 估算手数 >= 最小交易手数 (通常 0.01)。
          - **Fail**: 估算手数 < 0.01。 -> **Action 必须为 HOLD** (Risk budget too low)。
    """
    ...
    **平仓 (CLOSE_ALL) 标准**:
    ...
    3. **HOLD**:
       ...
       - **[NEW] 即使是 HOLD，也必须计算并返回当前最优的 sl_price (Smart Stop Loss)。系统会根据此价格自动更新现有持仓的止损位，以保护浮盈或限制亏损。**
    4. **CLOSE_ALL**:
    ...
    **一致性检查 (Consistency Check)**:
    ...
    - "telegram_report": str (Markdown 格式的简报。必须包含五个核心板块: 1.📊市场状态, 2.🚀策略模式(ORB/Grid), 3.📝盘前8问(简要回答), 4.🔭观察点, 5.⚖️仓位分析)
        """
    ...
    **Usage Instruction:**
    ...
    def __init__(self, api_key: str, base_url: str = "https://api.siliconflow.cn/v1", model: str = "Pro/zai-org/GLM-5"):
        ...
        eurusd_url = os.getenv("EURUSD_API_URL", base_url)
        eurusd_model = os.getenv("EURUSD_MODEL", model)
        
        # 加载环境变量中的 ETHUSD 配置
        ethusd_key = os.getenv("ETHUSD_API_KEY", api_key)
        ethusd_url = os.getenv("ETHUSD_API_URL", base_url)
        ethusd_model = os.getenv("ETHUSD_MODEL", model)

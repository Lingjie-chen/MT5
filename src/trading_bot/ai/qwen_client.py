...
class QwenClient:
    ...
    **关键规则：你的交易周期为 6分钟 (M6)。你必须结合 1小时 (H1) 和 4小时 (H4) 的大周期趋势来制定 M6 的入场决策。**
    ...
    *   **Position Sizing**: **完全由大模型分析判断**。你必须基于 M6/H1 的市场情绪和技术形态，计算出精确的仓位 (Lots)。

    1. **SMC (Smart Money Concepts) - 入场与方向**:
       - **方向判断**: 依据 H1/H4 确定主趋势，在 M6 寻找结构破坏(BOS)或特性改变(CHoch)。
       - **关键区域**: 重点关注 M6 和 H1 的订单块(Order Block)和失衡区(FVG)。
    ...
    **1. 分析师团队 (The Analyst Team)**
    ...
    - **技术分析师**:
        ...
        - **指令**:
            1. **SMC 结构分析**: 识别 M6/H1 的 BOS (结构破坏) 和 CHOCH (特性改变)。

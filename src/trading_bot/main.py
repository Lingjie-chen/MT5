...
import threading
import warnings
# Suppress joblib resource_tracker warnings (Windows specific cleanup race conditions)
# Even with threading backend, existing temp files from previous runs might trigger cleanup warnings
warnings.filterwarnings("ignore", message=".*resource_tracker.*")
warnings.filterwarnings("ignore", message=".*leaked folder objects.*")
try:
    from joblib.externals.loky import resource_tracker
    # Monkey-patch or configure resource_tracker if possible (advanced)
    # But usually filterwarnings is enough.
except ImportError:
    pass
...
def run_orb_simulation(params, df_h1):
    """
    Standalone function for ORB Strategy simulation.
    This function is picklable and can be used with joblib.Parallel.
    ...
    try:
        ...
        # p_grid_step = int(params[2]) # Not used in simple ORB sim
        # p_grid_tp = float(params[3]) # Not used in simple ORB sim
        ...
        # Ensure df_h1 is ready (pre-processed by caller if possible to avoid overhead here)
        # But we do checks to be safe
        if not pd.api.types.is_datetime64_any_dtype(df_h1['time']):
             # This might be slow inside loop, caller should ideally handle it
             df_h1['time'] = pd.to_datetime(df_h1['time'])
            
        # Group by date
        # If 'date' column doesn't exist, create it
        if 'date' not in df_h1.columns:
        ...
        for day in days[5:]:
            day_data = df_h1[df_h1['date'] == day] # .copy() removed for speed if read-only
            ...
            # day_data.loc is slow if index is not unique or not sorted.
            # Assuming typical OHLCV data structure.
            # Faster slicing:
            ...
            # Vectorization opportunity here, but loop is okay for short daily sessions
        ...
        # print(f"Sim Error: {e}") # logger might not be available in worker
        return -9999
...
class SymbolTrader:
    ...
    def manage_positions(self, signal=None, strategy_params=None):
        ...
        if should_close_long:
            ...
            self.close_all_positions(direction='long', comment="Basket TP (Long)")
        ...
        if should_close_short:
            ...
            self.close_all_positions(direction='short', comment="Basket TP (Short)")
            ...
            if llm_smart_sl > 0:
                ...
                if update_sl:
                    ...
                    if dist_p > 50: # Minimum 50 points change to avoid noise
                        ...
                        # Use modify_position (standard method)
            ...
            if self.latest_strategy:
                 ...
                 if basket_tp_val > 0:
                      ...
                      if tp > 0: 
                           request['tp'] = 0.0
                           changed = True
                           logger.info(f"Basket Mode: Removing TP for #{pos.ticket} (Global TP: {basket_tp_val})")
    ...
    def evaluate_comprehensive_params(self, params, df, df_h1):
        """
        Wrapper for the standalone simulation function.
        Kept for backward compatibility if needed, but optimize() should use run_orb_simulation directly.
        """
            ...
            if True:
                ...
                if should_trade_analyze:
                    ...
                    if df is not None:
                        ...
                        # [MODIFIED] Handle ORB Signal (Dict, String, or None)
